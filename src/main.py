"""
Cart Multi-Agent Voice System — main entry point.
...
from src.graph import cart_graph
...
class CartAssistant:
    ...
    def query(self, text: str, input_fn=None) -> str:
        ...
        while "__interrupt__" in result and result["__interrupt__"]:
            ...
            result = cart_graph.invoke(Command(resume=user_answer), config)
    ...
    def text_loop(self) -> None:
        ...
        print("\n🛒  Cart Assistant  (type 'quit' to exit)\n")
        while True:
            try:
    ...
    def voice_loop(self, max_turns: int = 10) -> None:
        ...
        welcome = "Hello! I'm your Cart assistant. How can I help you today?"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cart Multi-Agent Voice System")
    ...
    assistant = CartAssistant(enable_voice=args.voice)
