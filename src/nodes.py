...
from src.state import CartState, ClassificationResult, WorkerInput
...
PRODUCT_PROMPT = """\
You are the Product Discovery Agent for Cart.
...
SUPPORT_PROMPT = f"""\
You are the Sales Support Agent for Cart.
...
def orchestrator_node(state: CartState) -> Command[Literal["product_agent", "support_agent", "synthesizer"]]:
...
def synthesizer_node(state: CartState) -> dict:
    ...
    prompt = (
        ...
        "customer's query. Be concise. Speak as 'Cart Assistant'."
    )
...
if __name__ == "__main__":
    png_data = product_subgraph.get_graph().draw_mermaid_png()

        # Save it to a file
    with open("product_subgraph.png", "wb") as f:
        f.write(png_data)

        # Optionally display it in a notebook
    display(Image(filename="product_subgraph.png"))

    png_data = support_subgraph.get_graph().draw_mermaid_png()

    # Save it to a file
    with open("support_subgraph.png", "wb") as f:
        f.write(png_data)

        # Optionally display it in a notebook
    display(Image(filename="support_subgraph.png"))